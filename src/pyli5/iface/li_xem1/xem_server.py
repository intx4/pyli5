...
class XEMPipe:
    ...
    def push(self, msg: XEMMessage):
        """ Iface to logic"""
        self.upL.put(msg)

    def get(self)->XEMMessage:
        """ Logic reads from iface"""
        return self.upL.get()



class XEMServer(ThreadingHTTPServer):
    """
    ...
    """
    def __init__(self, addr: str, port: int, serving_path: str, xem_pipe: XEMPipe, logger):
        self.addr = addr
        self.port = port
        self.serving_path = serving_path
        self.x1_parser = X1Parser()
        self.xem_pipe = xem_pipe
        self.logger = logger
        super().__init__((addr, int(port)), RequestHandlerClass=XEMHandler)
        self.logger.log(f"  LI_XEM1_Server - started at {addr}:{port}")

class XEMHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        self.server.logger.log(f"  LI_XEM1_Server - received POST request {post_data.decode()}")
        try:
            msg = self.server.x1_parser.parse_X1_Message(post_data)

            q=Queue(1)
            self.server.xem_pipe.push(XEMMessage(msg=msg, responseQ=q))
            ...
            if "TopLevel" in ex.type_for_error:
                ...
                err_msg = X1Message(entity=SimpleResponse(ex.admf_id, ex.ne_id, ex.x1_tid, None, error=str(ex), code=ex.code, type_for_error=ex.type_for_error, type="ErrorResponse"))
                resp_code, resp_content = 500, etree.tostring(err_msg.xml_encode())
        except Exception as ex:
            self.server.logger.error(f" LI_XEM1_Server - panic: {str(ex)}")
            resp_code, resp_content = 500, b"error"
        finally:
            self.server.logger.log(f"  LI_XEM1_Server - sending response to POST {resp_code} : {resp_content.decode()}")
            if resp_code != 0:
