...
class HIQRClient():
    ...
    def send(self, host: str, port: int, url: str, content: str, timeout:float=None)->HI1_Message:
        try:
            ...
            self.logger.log(f"   LI_HIQR_Client - sending {content} to {host}:{port} at {url}")
            response = connection.getresponse()
            resp = self.h1_parser.parse_HI1Message(response.read())
            self.logger.log(f"  LI_HIQR_Client - receiving from {etree.tostring(resp.xml_encode(), pretty_print=True)} {host}:{port} at {url}")
            connection.close()
            return resp
